...
function ($rootScope, $scope, $controller, $timeout, DailyCRUDService, Utility, NOTIFYTYPE, NotificationService, commonValidationService, JobCRUDService, AppConfig, $filter, hylanCache, ngDialog) {
    ...
    function init() {
        ...
        $scope.DailyTypeWorkFlow["Radio Retro"] = [pages.dailyjob, pages.notes, pages.manpower, pages.vehicles, pages.workdetail];
    ...
    function Cell(val, backgroundColor, vAlign, hAlign, bold,showBorder,colSpan) {
        this.colSpan = 1;
        this.colSpan = colSpan != null && colSpan != '' ? colSpan : 1; //black
    ...
    //    var ProjectStatusDS = result;
    //    var ManpowerJobTypes = $.grep(ProjectStatusDS, function (lookup) {
    //        if (lookup.LU_TYPE === 'PROJECT_STATUS' && lookup.LU_NAME === "Closed")
    //        {
    //            $scope.Project_Status_Closed_ID = lookup.LOOK_UP_ID;
    //        }
    //    });
    //});
    ...
    Globals.GetProjects().then(function (result) {
        ...
        if ($scope.ngDialogData.DAILY_ID === 0) //add mode then remove projects with status code = closed
        {
            var projectsListOtherThanClosed = $.grep(result.objResultList, function (p) {
                return p.PROJECT_STATUS_LU.LU_NAME != "Closed"
            });
            $scope.DSProjects = projectsListOtherThanClosed;
        }
        else {
            $scope.DSProjects = result.objResultList;
        }

        $scope.DSProjects = $filter('orderBy')($scope.DSProjects, 'HYLAN_PROJECT_ID');
    ...
    $scope.ValidateAndPostData = function (thisForm, fromBtnSave, sender, prmNextPageId) {
        ...
        if (sender == 'repeat') {
            ...
            //check whether daily is closed or not
            if (isProjectClosed($scope.dailyDCModel.PROJECT_ID))
            {
                alert("You can't create another daily for a closed project.");
                return;
            }
    ...
    {
        var isValid = false;
        var projectsList = $.grep($scope.DSProjects, function (p) {
            if (p.PROJECT_ID === projectID)
            {
                if (p.PROJECT_STATUS_LU.LU_NAME === "Closed")
                {
                    isValid = true;
                    return;
                }
    ...
    $scope.ExportWholeDaily = function (e) {
        ...
        else if (dailyTypeText1 == "Node Install" || dailyTypeText1 == "Radio Retro") {
    ...
    //###############Export WholeDaily at one sheet start #########################
    function getManpowerSheet1(ManPowerList, sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "MANPOWER";
        //sheet1.columns = [{ width: 300 }, { width: 200 }, { width: 150 }, { width: 150 }, { width: 150 }];

        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("MANPOWER", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getManpowerWithJobTypeSheet1(ManPowerList, sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "MANPOWER";
        //sheet1.columns = [{ width: 300 }, { width: 200 }, { width: 150 }, { width: 150 }, { width: 150 }];
    ...
    function getLaborSheet1(LaborList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "LABOR";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }];
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("LABOR", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getVehicleSheet1(VehicleList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "VEHICLE";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 300 }, { width: 180 }];
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("VEHICLE", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getVehicle3ColumnSheet1(VehicleList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "VEHICLE";
        //sheet1.columns = [{ width: 300 }, { width: 300 }, { width: 180 }];
    ...
    function getLaborItemSheet1(LaborItemsList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "LABORITEM";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        ...
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("LABORITEM", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getMaterialTwoColumnSheet1(MaterialList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "MATERIALS";
        //sheet1.columns = [{ width: 300 }, { width: 150 }];
        ...
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("MATERIALS", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getMaterialSheet1(MaterialList, dailyType,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "MATERIALS";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
    ...
    function getAerialSheet1(MaterialList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "AERIAL";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        ...
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("AERIAL", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getMDUSheet1(MDUList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "MDU";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("MDU", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getWorkDetailForPoleWork1(WorkDetailList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "WORK DETAIL";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        sheet1.rows.push(new Row()); //push empty row
        var titleRow = new Row();
        var cells = [new Cell("WORK DETAIL", headerColor)];
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
    ...
    function getWorkDetailWithQtySheet1(WorkDetailList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "WORK DETAIL";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        sheet1.rows.push(new Row()); //push empty row
    ...
    function getWorkDetailSheet1(WorkDetailList,sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "WORK DETAIL";
        //sheet1.columns = [{ width: 180 }, { width: 300 }, { width: 180 }, { width: 300 }];
        sheet1.rows.push(new Row()); //push empty row
    ...
    function getNotesSheet1(daily, sheet1) {
        //var sheet1 = new Sheet();
        //sheet1.title = "NOTES";
        //sheet1.columns = [{ width: 900 }];
        ...
        //----row 1
        dataRowPoleLocationLabel = new Row();
        var c1 = new Cell("Notes", headerColor);
        c1.colSpan = 3 
        dataRowPoleLocationLabel.cells = [c1];
        sheet1.rows.push(dataRowPoleLocationLabel);
        //----row 2
        dataRowPoleLocationValues = new Row();
        var Notes = $('#txtNotes').val();
        var c2 = new Cell(Notes);
        c2.colSpan=3
        dataRowPoleLocationValues.cells = [c2];//var cellwithHeight = { cells: [{ value: Notes,vAlign :"top",hAlign : "left" }], height: 200 }
        var cellwithHeight = { cells: dataRowPoleLocationValues.cells, height: 150 }
        sheet1.rows.push(cellwithHeight);

        //return sheet1;
    }
    function getJobSheet1(daily,sheet1) {
        var titleRow = new Row();
        var cells = [new Cell("JOB & DAILY", headerColor)];
        var titleRow = new Row();
        titleRow.cells = cells;
        sheet1.rows.push(titleRow);
        //----row 1 
    ...
    $scope.ExportWholeDaily1 = function (e) {
        var dailyTypeText1 = $("#ddlDailyType option:selected").text();
        ...
        var jobSheet = new Sheet();
        jobSheet.title = "WHOLE DAILY";
        jobSheet.columns = [{ width: 200 }, { width: 200 }, { width: 200 }, { width: 150 }, { width: 150 }];

        var jobSheet = getJobSheet1(dailyDTO.DAILYDC,jobSheet);
        var notesSheet = getNotesSheet1(dailyDTO.DAILYDC, jobSheet);
        //excelSheets.push(jobSheet);
        
        //excelSheets.push(notesSheet);
        if (dailyTypeText1 == "Fiber Splicing/Testing/Extras") {
            //$scope.DailyTypeWorkFlow["Fiber Splicing/Testing/Extras"] = [pages.dailyjob, pages.manpower, pages.vehicles, pages.materials, pages.workdetail];
            var manPowerSheet = getManpowerSheet1(dailyDTO.listMAN_POWERDC, jobSheet);
            var vechicleSheet = getVehicle3ColumnSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var materialSheet = getMaterialTwoColumnSheet1(dailyDTO.listMATERIALDC, jobSheet);
            var workDetailSheet = getWorkDetailWithQtySheet1(dailyDTO.listWORK_DETAILDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(vechicleSheet);
            //excelSheets.push(materialSheet);
            //excelSheets.push(workDetailSheet);
            ...
            var manPowerSheet = getManpowerSheet1(dailyDTO.listMAN_POWERDC, jobSheet);
            var vechicleSheet = getVehicle3ColumnSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var workDetailSheet = getWorkDetailSheet1(dailyDTO.listWORK_DETAILDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(vechicleSheet);
            //excelSheets.push(workDetailSheet);
        }
            ...
            var manPowerSheet = getManpowerSheet1(dailyDTO.listMAN_POWERDC, jobSheet);
            var vechicleSheet = getVehicle3ColumnSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var workDetailSheet = getWorkDetailForPoleWork1(dailyDTO.listWORK_DETAILDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(vechicleSheet);
            //excelSheets.push(workDetailSheet);
        }
            ...
            var manPowerSheet = getManpowerWithJobTypeSheet1(dailyDTO.listMAN_POWERDC, jobSheet);
            var laborSheet = getLaborSheet1(dailyDTO.listLABORDC, jobSheet);
            var vechicleSheet = getVehicleSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var materialSheet = getMaterialSheet1(dailyDTO.listMATERIALDC, dailyTypeText1, jobSheet);
            var workDetailSheet = getWorkDetailSheet1(dailyDTO.listWORK_DETAILDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(laborSheet);
            //excelSheets.push(vechicleSheet);
            ...
            var laborSheet = getLaborSheet1(dailyDTO.listLABORDC, jobSheet);
            var vechicleSheet = getVehicle3ColumnSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var laborItemSheet = getLaborItemSheet1(dailyDTO.listLaborItemDC, jobSheet);
            var materialSheet = getMaterialSheet1(dailyDTO.listMATERIALDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(laborSheet);
            //excelSheets.push(vechicleSheet);
            //excelSheets.push(laborItemSheet);
            ...
            var laborSheet = getLaborSheet1(dailyDTO.listLABORDC, jobSheet);
            var vechicleSheet = getVehicleSheet1(dailyDTO.listVEHICLEDC, jobSheet);
            var aerialSheet = getAerialSheet1(dailyDTO.listAerialDC, jobSheet);
            var materialSheet = getMaterialSheet1(dailyDTO.listMATERIALDC, jobSheet);
            var mduSheet = getMDUSheet1(dailyDTO.listMDUDC, jobSheet);
            //excelSheets.push(manPowerSheet);
            //excelSheets.push(laborSheet);
            //excelSheets.push(vechicleSheet);
            //excelSheets.push(aerialSheet);
    ...
    //###############Export WholeDaily at one sheet end #########################
...
function checkBoxColumnTemplate(data) {
    ...
    var dailyTypeText = $("#ddlDailyType option:selected").text();
    if ((dailyTypeText == "Node Install" || dailyTypeText == "Radio Retro") && data.DESCRIPTION == "Number of Nodes Installed") {
...
function editNumberWorkDetails(container, options) {
    var dailyTypeText = $("#ddlDailyType option:selected").text();
    if ((dailyTypeText == "Node Install" || dailyTypeText == "Radio Retro") && options.field == "PERFORMED" && options.model.DESCRIPTION == "Number of Nodes Installed") {
